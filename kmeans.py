...
def train_kmean_torch(train_data, k = 5, lr=0.1, epoch=150):
  ...
  for epoch in range(epoch):
    ...
    for i in range(k):
      ...
      # for every cluster mean, calculate squared distance between
      # data points and the cluster mean
      distance_array = torch.norm((X_train - m[i]), dim=1)
      squared_distance_array = torch.square(distance_array)
      list_mse.append(squared_distance_array)

    #TODO: calculate the loss function. Complete the 
    #below 3 lines. 
    list_mse_torch = torch.stack(list_mse, dim=0) #Stacks the list_mse[i]
...
def evaluate(test_data, m):
  ...
  for i in range(k):
    # for every cluster mean, calculate squared distance between
    # data points and the cluster mean
    distance_array = np.linalg.norm((test_data - m[i]), axis=1)
    squared_distance_array = np.square(distance_array)
    list_mse.append(squared_distance_array)

  list_mse_np = np.stack(list_mse, axis=0) #Stacks the list_mse[i]
  list_mse_np_min = np.min(list_mse_np, axis=0) #Calculate the min 
  L_train = np.mean(list_mse_np_min) #Calculate the mean loss
...
def get_association(test_data, m):
  ...
  for k in range(num_cluster):
    ...
    distance_array = np.linalg.norm((test_data - m[k]), axis=1)
    squared_distance_array = np.square(distance_array)
    L_k[:,k] = squared_distance_array
...
def test_sckitlearn(train_data, test_data, k=5):
  ...
  kmeans = KMeans(n_clusters=k, max_iter=5000, algorithm="lloyd")
